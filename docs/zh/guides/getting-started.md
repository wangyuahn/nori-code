...
- **Nori Code（OAuth）** — 验证码流程，在任意设备打开链接、登录并输入验证码即可授权
- **Nori Platform API 密钥** — 从平台控制台获取 API 密钥
...
Kimi Code CLI 会自动调用文件读取、搜索等工具浏览相关内容后给出回答。只读操作默认自动执行无需确认。主 Agent 也可以在普通权限流程下运行有边界的 `Bash` 检查；当 Nori 只读模式开启时，直接 `Write` 和 `Edit` 会被拦截。
