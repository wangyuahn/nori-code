...
`/team` 打开某个成员自己的会话（消息和输入都针对该成员，受 Discuss / 权限限制）。在 Main 行按 Enter 回到主会话。

Discuss 开启时，下方 Discuss 栏是只读开会轨（谁在说、说了什么）；其余时候显示部门 Chat。`Ctrl-Y` 或空闲时 `Esc` 可隐藏该栏；关栏不会退出 Discuss，也不会退出当前成员会话。
