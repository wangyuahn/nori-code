...
Nori Code is a loop-core multi-agent coding tool. The orchestrator is read-only — it searches project memory, enforces custom rules, and delegates all code changes to parallel SubAgents.
...
- **SubAgents and graph checks** — parallel delegated sessions plus DAG-based execution with recursive depth control
- **Read-only orchestrator** — forced delegation via `SubAgent`, with configured graph checks available through `nori_swarm_launch`
...
- **12-setting GUI** — model, permission, theme, SubAgent depth, coder write, note rules, workflow thresholds
