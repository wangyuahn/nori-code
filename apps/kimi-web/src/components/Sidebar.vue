...
// Logo easter-egg: clicking the Nori mark plays one quick pulse. It's a one-shot
// animation — force a reflow so rapid clicks restart it, then drop the class so
// the idle node animation resumes.
const logoRef = ref<{ blink: () => void } | null>(null);

// Keep workspace creation reachable from the primary navigation surface.
...
// Logo long-press easter-egg: holding the Nori mark for 1 second opens the
    ...
    <div class="col" :style="{ width: colWidth + 'px' }">
      ...
      <div class="ch">
        <div class="ch-brand">
          ...
          />
          <span class="ch-name">{{ t('sidebar.brand') }}<span v-if="isDev" class="ch-endpoint"> · {{ endpoint }}</span></span>
          <InternalBuildBanner />
        </div>
