...
function validBundle(): Record<string, string> {
  return {
    ...
    'logs/nori-code.log': '2026-06-01T00:00:00.000Z INFO  hello  k=v\n',
  };
}
...
describe('import-store', () => {
  ...
  it('imports a valid bundle and lists it with manifest metadata', async () => {
    ...
    expect((await stat(join(dir, 'logs', 'nori-code.log'))).isFile()).toBe(true);
...
describe('resolveSafeTarget (zip-slip guard)', () => {
  ...
  it('accepts in-tree paths', () => {
    expect(resolveSafeTarget(root, 'state.json')).toBe(resolve(root, 'state.json'));
    expect(resolveSafeTarget(root, 'agents/main/wire.jsonl')).toBe(
      resolve(root, 'agents/main/wire.jsonl'),
    );
  });
