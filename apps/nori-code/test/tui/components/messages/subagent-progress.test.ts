...
import {
  ...
  subagentDescriptionFromArgs,
  subagentGridHeightForTerminalRows,
  subagentItemsFromArgs,
  subagentPartialItemsCountFromArguments,
  subagentPartialItemsFromArguments,
  calculateSubAgentGridLayout,
} from '#/tui/components/messages/subagent-progress';
import { SubAgentProgressEstimator } from '#/tui/components/messages/subagent-progress-estimator';
...
function createComponent(
  options: Partial<SubAgentProgressOptions> = {},
): SubAgentProgressComponent {
  return new SubAgentProgressComponent({
...
function renderText(component: SubAgentProgressComponent, width = 100): string {
  return strip(component.render(width).join('\n'));
}

function renderLines(component: SubAgentProgressComponent, width = 100): string[] {
  return renderText(component, width).split('\n');
}

function registerSubagents(component: SubAgentProgressComponent, count: number): void {
...
function startSubagents(component: SubAgentProgressComponent, count: number): void {
...
describe('calculateSubAgentGridLayout', () => {
  ...
  it('adds text columns before falling back to compact bars', () => {
    const textLayout = calculateSubAgentGridLayout({
      width: 120,
      height: 4,
      count: 20,
    });
    const compactLayout = calculateSubAgentGridLayout({
      width: 117,
      height: 4,
      count: 20,
    });
  ...
  it('derives the grid height left inside the SubAgent block', () => {
    expect(subagentGridHeightForTerminalRows(undefined)).toBeUndefined();
    expect(subagentGridHeightForTerminalRows(10)).toBe(4);
    expect(subagentGridHeightForTerminalRows(20, 5)).toBe(9);
    expect(subagentGridHeightForTerminalRows(4)).toBe(0);
  });
});

describe('SubAgentProgressComponent', () => {
  ...
  it('repaints from the active palette when the theme changes', () => {
    ...
    try {
      ...
      const titleOf = (): string => {
        const line = component.render(100).find((l) => strip(l).includes('SubAgent'));
  ...
  it('reserves one blank column on the right edge', () => {
    ...
    expect(rendered.some((line) => line.includes('SubAgent'))).toBe(true);
  ...
  it('renders failure details from SubAgent result output', () => {
    const component = createComponent();
  ...
  it('renders graph-check result output', () => {
    const component = createComponent();
    ...
    component.applyResult([
      '<graph_check_result>',
      '<summary>failed: 1</summary>',
      '<subagent index="1" agent_id="agent-1" outcome="failed">Graph check timed out.</subagent>',
      '</graph_check_result>',
    ].join('\n'));

    const output = renderText(component);

    expect(output).toContain('✗ Graph check timed out.');
    expect(output).not.toContain('Failed:');
  });

  it('applies no-index SubAgent result statuses by tag order', () => {
    const component = createComponent();
  ...
  it('strips nested SubAgent prefixes from failure details', () => {
    const component = createComponent();
    ...
    component.applyResult([
      '<subagent_result>',
      '<summary>failed: 1</summary>',
      '<subagent index="1" agent_id="agent-1" outcome="failed">subagent: failed',
  ...
  it('renders completed summaries from SubAgent result output', () => {
    const component = createComponent();
  ...
  it('keeps a two-cell placeholder after the SubAgent tool call ends', () => {
    const component = createComponent();
  ...
  it('counts partial items before each string is complete', () => {
    expect(
      subagentPartialItemsCountFromArguments('{"items":["src/a.ts","src/b'),
    ).toBe(2);
    expect(
      subagentPartialItemsCountFromArguments('{"items":["src/a.ts","src/\\"b.ts","src/c'),
    ).toBe(3);
    expect(
      subagentPartialItemsFromArguments('{"items":["src/a.ts","src/\\"b.ts","src/c'),
  ...
  it('maps subagents by structured task indexes when descriptions include issue references', () => {
    const component = createComponent({
      description: 'Fix #123',
    });
  ...
  it('extracts description and item list from SubAgent args', () => {
    const args = {
      description: 'Review changed files',
      items: ['src/a.ts', 123],
    };

    expect(subagentDescriptionFromArgs(args)).toBe('Review changed files');
    expect(subagentItemsFromArgs(args)).toEqual(['src/a.ts', '123']);
  });
});

describe('SubAgentProgressEstimator', () => {
  it('counts a started subagent as one progress tick before tool calls arrive', () => {
    const estimator = new SubAgentProgressEstimator();
