...
describe('runPrompt', () => {
  beforeEach(() => {
    process.env['NORI_CODE_HOME'] = '/tmp/nori-code-test-home';
  });
  afterEach(() => {
  ...
  it('creates a fresh auto-permission session and streams assistant output to stdout', async () => {
    ...
    expect(stderr.text()).toBe('To resume this session: nori -r ses_prompt\n');
  ...
  it('formats thinking and assistant output as transcript blocks', async () => {
    ...
    expect(stderr.text()).toBe(
      '• The user wants an exact reply.\n  No tools are needed.\n\nTo resume this session: nori -r ses_prompt\n',
    );
  ...
  it('writes stream-json output as assistant JSONL with resume meta without transcript bullets', async () => {
    ...
    expect(stdout.text()).toBe(
      [
        ...
        '{"role":"meta","type":"session.resume_hint","session_id":"ses_prompt","command":"nori -r ses_prompt","content":"To resume this session: nori -r ses_prompt"}',
        '',
      ].join('\n'),
    );
    expect(stderr.text()).toBe('');
