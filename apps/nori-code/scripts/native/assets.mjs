...
async function collectPackageFiles({
  ...
  for (const runtimeFileRelative of runtimeFileRelatives) {
    const runtimeFile = resolve(packageRoot, runtimeFileRelative);
    if (!existsSync(runtimeFile)) {
      fail(`Native package ${packageName} does not contain ${runtimeFileRelative} at ${packageRoot}`);
    }
    selected.add(runtimeFile);
    await addRuntimeDependencyFiles(packageRoot, runtimeFile, selected);
  }
...
async function packageManifestEntries({
  ...
  const assets = {};
  const executableFiles = new Set(executableFileRelatives.map(toPosixPath));
  ...
  for (const file of files) {
    ...
    const entry = {
      ...
      mode: executableFiles.has(packageRelativePath) ? 0o755 : undefined,
    };
    entries.push(entry);
