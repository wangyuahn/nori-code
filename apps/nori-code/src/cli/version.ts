...
/**
 * Product User-Agent (`nori-code-cli/<version>`) for ad-hoc outbound fetches
...
export function createKimiCodeUserAgent(version = getVersion()): string {
  return `${CLI_USER_AGENT_PRODUCT}/${version}`;
}

export function buildKimiDefaultHeaders(version: string): Record<string, string> {
  return { 'User-Agent': createKimiCodeUserAgent(version) };
}
