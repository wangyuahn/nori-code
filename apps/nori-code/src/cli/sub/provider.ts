...
export async function handleProviderAdd(
  ...
  if (apiKey === undefined) {
    deps.stderr.write(
      'Missing API key. Pass --api-key <key> or set NORI_REGISTRY_API_KEY.\n',
    );
    deps.exit(1);
  }
...
export async function handleCatalogAdd(
  ...
  if (opts.defaultModel !== undefined && !models.some((m) => m.id === opts.defaultModel)) {
    deps.stderr.write(
      `Model "${opts.defaultModel}" is not in provider "${providerId}". Run "nori provider catalog list ${providerId}" to see available ids.\n`,
    );
    deps.exit(1);
  }
...
export function registerProviderCommand(parent: Command, deps?: Partial<ProviderDeps>): void {
  ...
  provider
    ...
    .option('--api-key <key>', 'Registry API key. Falls back to NORI_REGISTRY_API_KEY.')
  ...
  catalog
    ...
    .option('--api-key <key>', 'API key for the provider. Falls back to NORI_REGISTRY_API_KEY.')
...
function resolveApiKey(flag: string | undefined, env: NodeJS.ProcessEnv): string | undefined {
  ...
  const fromEnv = env['NORI_REGISTRY_API_KEY'];
