import { createKimiDeviceId } from '@nori-code/oauth';
import {
...
/**
 * Bootstrap telemetry for the `nori web` / `nori server run` host.
 *
 * Mirrors {@link initializeCliTelemetry}: mints the device id, reads config for
 * the default model, and attaches a permanently disabled local telemetry client.
 ...
 *
 * The returned client wraps the `@nori-code/telemetry` module
...
function readServerTelemetryConfig(
  ...
  try {
    ...
    // telemetry just degrades to no model so it never blocks server startup.
