...
import {
  SubAgentProgressEstimator,
  type SubAgentProgressEstimatorPhase,
} from '#/tui/components/messages/subagent-progress-estimator';
...
const SUBAGENT_LEFT_INDENT = ' ';
const SUBAGENT_RIGHT_GAP = 1;
const SUBAGENT_NON_GRID_LINES = 6;
...
const SUBAGENT_TITLE_ACCENT_BIAS = 1.3;
...
type SubAgentPhase = SubAgentProgressEstimatorPhase;
...
interface SubAgentResultStatus {
...
export interface SubAgentGridLayoutInput {
  readonly width: number;
  readonly height: number;
  readonly count: number;
}

export interface SubAgentGridLayout {
...
export class SubAgentProgressComponent implements Component {
  private members: SubAgentMember[];
  private readonly progressEstimator = new SubAgentProgressEstimator();
  ...
  updateArgs(
    ...
    const description = subagentDescriptionFromArgs(args);
    if (description.length > 0 || this.description.length === 0) {
      this.description = description;
    }
    const fullRows = [...subagentResumeItemsFromArgs(args), ...subagentItemsFromArgs(args)];
    const partialRows = streamingArguments === undefined
      ? []
      : [
          ...subagentPartialResumeItemsFromArguments(streamingArguments),
          ...subagentPartialItemsFromArguments(streamingArguments),
        ];
    if (
      fullRows.length > 0 ||
      partialRows.length > 0 ||
      (streamingArguments !== undefined && subagentWorkItemsStartedFromArguments(streamingArguments))
    ) {
      this.itemsStarted = true;
    }
    const fullPromptTemplate = subagentPromptTemplateFromArgs(args);
    const partialPromptTemplate =
      streamingArguments === undefined
        ? ''
        : subagentPartialPromptTemplateFromArguments(streamingArguments);
  ...
  registerSubagent(input: {
    ...
    const member = this.findMemberForSubagent(input.agentId, input.subagentIndex);
  ...
  markSuspended(input: {
    ...
    const member = this.findMemberByAgentId(input.agentId) ??
      this.findMemberForSubagent(input.agentId, input.subagentIndex);
  ...
  markSubagentFailed(failureText?: string): void {
    this.failed = true;
    this.aborted = false;
    const nowMs = Date.now();
    for (const member of this.members) {
  ...
  applyResult(output: string): boolean {
    const statuses = parseSubAgentResultStatuses(output);
  ...
  render(width: number): string[] {
    ...
    const innerWidth = Math.max(
      1,
      outerWidth - visibleWidth(SUBAGENT_LEFT_INDENT) - SUBAGENT_RIGHT_GAP,
    );
    ...
    const nowMs = Date.now();
    const snapshots = this.members.map((member): SubAgentSnapshot => ({
      phase: member.phase,
      ticks: member.ticks,
  ...
  private indentLines(lines: readonly string[], width: number): string[] {
    const contentWidth = Math.max(
      0,
      width - visibleWidth(SUBAGENT_LEFT_INDENT) - SUBAGENT_RIGHT_GAP,
    );
    return lines.map((line) =>
      truncateToWidth(
        SUBAGENT_LEFT_INDENT + truncateToWidth(line, contentWidth),
        width,
      )
    );
  }

  private renderHeader(width: number, _summary: SubAgentSummary | undefined): string {
    if (width <= 3) return chalk.hex(this.colors.primary)('─'.repeat(width));

    const title = gradientText('SubAgent', this.colors.primary, this.colors.accent, SUBAGENT_TITLE_ACCENT_BIAS);
    const description =
  ...
  private renderGrid(
    ...
    const layout = calculateSubAgentGridLayout({
      width,
  ...
  private findMemberForSubagent(
    agentId: string,
    subagentIndex: number | undefined,
  ): SubAgentMember | undefined {
    const existing = this.findMemberByAgentId(agentId);
    if (existing !== undefined) return existing;

    if (subagentIndex !== undefined && Number.isInteger(subagentIndex) && subagentIndex > 0) {
      this.ensureMemberCount(subagentIndex);
      const byIndex = this.members[subagentIndex - 1];
  ...
  private findMemberByAgentId(agentId: string): SubAgentMember | undefined {
  ...
  private promoteToRunning(member: SubAgentMember, nowMs?: number, setTicks = false): void {
  ...
  private completeMember(member: SubAgentMember, nowMs: number, completedText?: string): void {
  ...
  private failMember(member: SubAgentMember, nowMs: number, failureText?: string): void {
  ...
  private cancelMember(member: SubAgentMember, nowMs: number): void {
...
function createMembers(count: number, phase: SubAgentPhase): SubAgentMember[] {
...
function clearMemberState(member: SubAgentMember, ...keys: ClearableMemberKey[]): void {
  for (const key of keys) delete member[key];
}

function isTerminalPhase(phase: SubAgentPhase): boolean {
  return phase === 'completed' || phase === 'failed' || phase === 'cancelled';
}

function terminalPhaseElapsedMs(member: SubAgentMember, nowMs: number): number {
...
export function subagentItemsFromArgs(args: Record<string, unknown>): string[] {
  const items = args['items'];
  if (!Array.isArray(items)) return [];
  return items.map(String);
}

function subagentResumeItemsFromArgs(args: Record<string, unknown>): string[] {
...
export function subagentPartialItemsCountFromArguments(argumentsText: string): number {
  return subagentPartialItemsFromArguments(argumentsText).length;
}

function subagentWorkItemsStartedFromArguments(argumentsText: string): boolean {
  return /"items"\s*:/.test(argumentsText) || /"resume_agent_ids"\s*:/.test(argumentsText);
}

export function subagentPartialItemsFromArguments(argumentsText: string): string[] {
...
function subagentPartialResumeItemsFromArguments(argumentsText: string): string[] {
...
export function subagentDescriptionFromArgs(args: Record<string, unknown>): string {
  const description = args['description'];
  return typeof description === 'string' ? description : '';
}

function subagentPromptTemplateFromArgs(args: Record<string, unknown>): string {
  const promptTemplate = args['prompt_template'];
  return typeof promptTemplate === 'string' ? promptTemplate : '';
}

function subagentPartialPromptTemplateFromArguments(argumentsText: string): string {
...
export function subagentResultSummaryFromOutput(output: string): SubAgentResultSummary {
  const statuses = parseSubAgentResultStatuses(output);
  let completed = 0;
  let failed = 0;
  let aborted = 0;
...
function parseSubAgentResultStatuses(output: string): SubAgentResultStatus[] {
  const xmlStatuses = parseSubAgentXmlResultStatuses(output);
  if (xmlStatuses.length > 0) return xmlStatuses;
  return parseSubAgentLegacyResultStatuses(output);
}
...
function parseSubAgentXmlResultStatuses(output: string): SubAgentResultStatus[] {
...
function parseSubAgentLegacyResultStatuses(output: string): SubAgentResultStatus[] {
  return forEachAgentBlock(output, (block, index) => {
    ...
    return {
      ...
      completedText: status === 'completed' ? parseSubAgentCompletedText(block) : undefined,
      failureText: status === 'failed' ? parseSubAgentFailureText(block) : undefined,
    };
  });
}

function parseSubAgentCompletedText(block: string): string | undefined {
  const marker = '\n[summary]\n';
...
function parseSubAgentFailureText(block: string): string | undefined {
...
export function calculateSubAgentGridLayout(
  input: SubAgentGridLayoutInput,
): SubAgentGridLayout {
  const count = Math.max(0, Math.floor(input.count));
  const width = Math.max(0, Math.floor(input.width));
  const height = Math.max(0, Math.floor(input.height));
  const idWidth = subagentGridIdWidth(count);
...
export function subagentGridHeightForTerminalRows(
  rows: number | undefined,
  followingRows = 0,
): number | undefined {
  if (rows === undefined || !Number.isFinite(rows)) return undefined;
  const rowsAfterSubagent = Number.isFinite(followingRows)
    ? Math.max(0, Math.floor(followingRows))
    : 0;
  return Math.max(0, Math.floor(rows) - rowsAfterSubagent - SUBAGENT_NON_GRID_LINES);
}

function subagentGridIdWidth(count: number): number {
...
function summarizeSnapshots(snapshots: readonly SubAgentSnapshot[]): SubAgentSummary {
  let completed = 0;
  let failed = 0;
  let cancelled = 0;
...
function brailleBar(
  ...
  const colorMap: Record<Exclude<SubAgentPhase, 'pending' | 'failed' | 'cancelled'>, string> = {
...
function phaseColor(phase: SubAgentPhase, colors: ColorPalette): string {
  const map: Record<SubAgentPhase, string> = {
...
function statusBarCounts(members: readonly SubAgentMember[]): StatusBarCount[] {
...
function statusBarPhase(phase: SubAgentPhase): StatusBarPhase {
  const map: Record<SubAgentPhase, StatusBarPhase> = {
    pending: 'queued',
    queued: 'queued',
    suspended: 'suspended',
    running: 'working',
...
function runningCellLabelText(member: SubAgentMember): string {
...
function normalizeFailureText(text: string | undefined): string | undefined {
  ...
  const nestedFailureText = nestedSubAgentFailureText(text);
  const normalized = stripSubAgentPrefix(collapseWhitespace(nestedFailureText ?? text));
  return normalized.length > 0 ? normalized : undefined;
}

function nestedSubAgentFailureText(text: string): string | undefined {
  const xmlFailureText = nestedSubAgentXmlFailureText(text);
  if (xmlFailureText !== undefined) return nestedSubAgentFailureText(xmlFailureText) ?? xmlFailureText;

  if (!/^\s*subagent:\s*failed\b/m.test(text)) return undefined;
  const match = /^\s*subagent error:\s*([\s\S]*?)(?=\n\[agent \d+\]\n|$)/m.exec(text);
  if (match === null) return undefined;
  const failureText = match[1];
  if (failureText === undefined) return undefined;
  return nestedSubAgentFailureText(failureText) ?? failureText;
}

function nestedSubAgentXmlFailureText(text: string): string | undefined {
  if (!/<(?:subagent_result|graph_check_result)\b/.test(text)) return undefined;
  const failed = parseSubAgentXmlResultStatuses(text).find((entry) => {
    return entry.status === 'failed' && entry.failureText !== undefined;
  });
  return failed?.failureText;
}

function stripSubAgentPrefix(text: string): string {
  return text.replace(/^subagent:\s*(?:failed|completed)?\s*/i, '').trim();
}
