...
export class MigrationScreenComponent extends Container implements Focusable {
  ...
  private renderResult(width: number): string[] {
    ...
    if (this.migrationFailed) {
      ...
      lines.push('');
      lines.push(chalk.hex(colors.text)(' You can retry later by running "nori migrate".'));
      lines.push('');
      lines.push(chalk.hex(colors.textMuted)(' ⏎ continue to nori-code'));
    ...
    if (r !== undefined) {
      ...
      // landed in a *.migrated-from-legacy-cli.* sibling or that sessions failed.
      ...
      if (sum.config.wroteSiblingDueToConflict) {
        ...
        // migrated content went to `config.migrated-from-legacy-cli.toml` and
        ...
        lines.push(
          chalk.hex(colors.warning)(
            '  ⚠ config.toml could not be parsed — review config.migrated-from-legacy-cli.toml',
          ),
        );
      ...
      if (sum.config.wroteTuiSibling) {
        lines.push(
          chalk.hex(colors.warning)(
            '  ⚠ tui.toml conflicted — review tui.migrated-from-legacy-cli.toml',
          ),
        );
      }
      if (sum.mcp.wroteSiblingDueToConflict) {
        lines.push(
          chalk.hex(colors.warning)(
            '  ⚠ mcp.json unreadable — review mcp.migrated-from-legacy-cli.json',
          ),
        );
      }
...
function stepFor(phase: Phase, plan: MigrationPlan): StepDef {
  if (phase === 'ask1') {
    return {
      title: 'Migrate this data to nori-code?',
      options: [
