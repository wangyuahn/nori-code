...
function shouldAlwaysBundle(id: string): boolean {
  ...
  // Everything else is force-bundled, which covers `@nori-code/*` (incl.
...
function buildTarget(): string {
  return process.env['NORI_CODE_BUILD_TARGET'] ?? `${process.platform}-${process.arch}`;
}
...
export default defineConfig({
  ...
  define: {
    ...
    __KIMI_CODE_CHANNEL__: JSON.stringify(process.env['NORI_CODE_CHANNEL'] ?? ''),
    __KIMI_CODE_COMMIT__: JSON.stringify(process.env['NORI_CODE_COMMIT'] ?? ''),
