...
export function ApprovalPanel({ requests, onResolve, onPermissionChange, browserPermissions = [], onResolveBrowserPermission, floating = false, sessionTitles = {}, onOpenSession, resolvingIds = new Set(), errors = {} }: ApprovalPanelProps) {
  ...
  const renderRequest = (request: ApprovalRequest) => {
    ...
    return <div key={request.approval_id} className={`approval-card pending approval-kind-${kind ?? 'generic'}`} aria-busy={isResolving}>
      <div className="approval-card-header">
        <span className="approval-tool-icon"><Icon name={kind === 'goal_start' ? 'target' : 'settings'} size={15}/></span>
        <span className="approval-tool-name">{kind === 'goal_start' ? tr('Start goal', '启动目标') : request.tool_name}</span>
        <span className="approval-status approval-status--pending">{tr('Permission required', '需要授权')}</span>
      </div>
      {(floating || onOpenSession !== undefined) && (
        <div className="approval-source">
          <span>
            {sessionTitles[request.session_id] || request.session_id}
            {request.agent_id && request.agent_id !== 'main' ? ` · ${request.agent_id}` : ''}
          </span>
          {onOpenSession && (
            <button type="button" onClick={() => onOpenSession(request.session_id, request.agent_id)}>
              {tr('Open source', '打开来源')}
            </button>
          )}
        </div>
      ...
      {kind === 'generic' && (
        <textarea
          ...
          onChange={event => setFeedback(previous => ({ ...previous, [request.approval_id]: event.target.value }))}
          placeholder={tr('Optional feedback', '可选反馈')}
        />
      )}

      {(modeErrors[request.approval_id] || errors[request.approval_id]) && (
        <div className="approval-mode-error" role="alert">{modeErrors[request.approval_id] || errors[request.approval_id]}</div>
      )}
      ...
      <div className="approval-actions">
        {kind === 'goal_start' ? (
          <GoalApprovalActions
            display={display}
            onSelect={(decision, label) => { void resolve(decision, { selectedLabel: label }); }}
            tr={tr}
          />
        ) : (
          <>
            <button className="approval-btn approval-btn--approve" disabled={Boolean(activeModeSwitch)} onClick={() => { void resolve('approved'); }}>
              {tr('Approve', '允许')}
            </button>
            {onPermissionChange && (
              <>
            ...
            <button className="approval-btn approval-btn--decline" disabled={Boolean(activeModeSwitch)} onClick={() => { void resolve('rejected'); }}>
              {tr('Decline', '拒绝')}
            </button>
            <label className="approval-always-allow">
              <input
                type="checkbox"
                checked={remember}
                disabled={Boolean(activeModeSwitch)}
                onChange={event => setRemembered(previous => {
                  const next = new Set(previous);
                  if (event.target.checked) next.add(request.approval_id);
                  else next.delete(request.approval_id);
                  return next;
                })}
              />
              {tr('Always allow this tool in this session', '本会话始终允许此工具')}
            </label>
          </>
        )}
      </div>
  ...
  const renderBrowserPermission = (request: NoriBrowserState['permissions']['pending'][number]) => {
    ...
    return <div key={request.id} className="approval-card pending browser-permission-card">
      <div className="approval-card-header">
        <span className="approval-tool-icon"><Icon name="globe" size={15}/></span>
        <span className="approval-tool-name">{tr('Browser permission', '浏览器权限')}</span>
        <span className="approval-status approval-status--pending">{tr('Permission required', '需要授权')}</span>
      </div>
      ...
      <div className="approval-actions">
        <button className="approval-btn approval-btn--approve" disabled={Boolean(activeModeSwitch)} onClick={() => { void onResolveBrowserPermission?.(request.id, 'allow_once'); }}>
          {tr('Allow once', '允许一次')}
        </button>
        {onPermissionChange && (
          <>
            <button
              className="approval-btn approval-btn--auto"
              disabled={Boolean(activeModeSwitch)}
              onClick={() => { void switchModeAndResolve(request.id, 'auto', () => onResolveBrowserPermission?.(request.id, 'allow_once')); }}
            >
              {activeModeSwitch === 'auto' ? tr('Switching…', '切换中…') : tr('Switch to AUTO and approve', '切换为 AUTO 并允许')}
            </button>
            <button
              className="approval-btn approval-btn--yolo"
              disabled={Boolean(activeModeSwitch)}
              onClick={() => { void switchModeAndResolve(request.id, 'yolo', () => onResolveBrowserPermission?.(request.id, 'allow_once')); }}
            >
              {activeModeSwitch === 'yolo' ? tr('Switching…', '切换中…') : tr('Switch to YOLO and approve', '切换为 YOLO 并允许')}
            </button>
          </>
        )}
        <button className="approval-btn approval-btn--approve" disabled={Boolean(activeModeSwitch)} onClick={() => { void onResolveBrowserPermission?.(request.id, 'allow_always'); }}>
          {tr('Always allow', '始终允许')}
        </button>
        <button className="approval-btn approval-btn--decline" disabled={Boolean(activeModeSwitch)} onClick={() => { void onResolveBrowserPermission?.(request.id, 'deny'); }}>
          {tr('Deny', '拒绝')}
        </button>
        <button className="approval-btn approval-btn--decline" disabled={Boolean(activeModeSwitch)} onClick={() => { void onResolveBrowserPermission?.(request.id, 'deny_always'); }}>
          {tr('Always deny', '始终拒绝')}
        </button>
      </div>
    </div>;
  ...
  return (
    <aside className={`approval-dock approval-dock-${layout}${floating ? ' approval-dock-floating' : ''}`} onWheel={onWheel}>
      <header className="approval-dock-header">
        <span>
          <Icon name="alert" size={14}/>
          <strong>{tr('Permissions', '授权')}</strong>
          <small>{items.length}</small>
        </span>
        <div className="approval-layout-switch" role="group" aria-label={tr('Approval layout', '授权布局')}>
          {floating && (
            <button
              type="button"
              onClick={() => setCollapsed(previous => !previous)}
              aria-expanded={!collapsed}
              aria-label={collapsed ? tr('Expand permissions', '展开授权') : tr('Collapse permissions', '收起授权')}
              title={collapsed ? tr('Expand permissions', '展开授权') : tr('Collapse permissions', '收起授权')}
            >
          ...
          <button
            ...
            title={tr('Group requests and use the mouse wheel to switch', '合并请求，用鼠标滚轮切换')}
          >
          ...
          <button
            ...
            title={tr('Show requests vertically', '纵向平铺请求')}
          >
      ...
      {!collapsed && (
        <>
          <div className="approval-panel">
            {layout === 'compact' ? renderItem(items[activeIndex]!) : items.map(renderItem)}
          </div>
          {layout === 'compact' && items.length > 1 && (
            <footer className="approval-pager">
              <button type="button" onClick={() => setActiveIndex(index => (index - 1 + items.length) % items.length)}>
                <Icon name="chevron-left" size={13}/>
                <span className="sr-only">{tr('Previous approval', '上一个授权请求')}</span>
              </button>
              <span>{activeIndex + 1} / {items.length} · {tr('Scroll to switch', '滚轮切换')}</span>
              <button type="button" onClick={() => setActiveIndex(index => (index + 1) % items.length)}>
                <Icon name="chevron-right" size={13}/>
                <span className="sr-only">{tr('Next approval', '下一个授权请求')}</span>
              </button>
            </footer>
          )}
        </>
...
function ApprovalDisplay({ display, fallback }: { display: DisplayData | null; fallback: unknown }) {
  if (display?.kind === 'goal_start') {
    return (
      <div className="approval-goal">
        <strong>{displayText(display.objective ?? '')}</strong>
        {typeof display.completionCriterion === 'string' && <span>{display.completionCriterion}</span>}
      </div>
    );
  }
  if (display?.kind === 'command') return <pre className="approval-args">{displayText(display.command ?? '')}</pre>;
  if (display?.kind === 'file_io' || display?.kind === 'diff') {
    return (
      <pre className="approval-args">
        {displayText(display.path ?? '')}
...
function GoalApprovalActions({ display, onSelect, tr }: { display: DisplayData | null; onSelect: (decision: 'approved' | 'cancelled', label: string) => void; tr: (en: string, zh: string) => string }) {
  ...
  return (
    <>
      {choices.map(choice => (
        <button key={choice.value} className="approval-btn approval-btn--approve" onClick={() => onSelect('approved', choice.value)}>
          {choice.label}
        </button>
      ))}
      <button className="approval-btn approval-btn--decline" onClick={() => onSelect('cancelled', 'cancel')}>
        {tr('Do not start', '不启动')}
      </button>
    </>
  );
