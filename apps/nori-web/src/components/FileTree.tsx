  ...
  return <section className="file-explorer">
    ...
    <div className="file-tree" role="tree">
      {!sessionId ? <div className="file-tree-state">{tr('Select a session to browse its project.', '选择会话以浏览对应项目。')}</div>
        ...
        : rootEntries.map(entry => <FileTreeNode key={entry.path} entry={entry} depth={0} selectedPath={selectedPath} onSelectFile={onSelectFile} readDir={filesystem.readDir} />)}
    </div>
  </section>;
}

function FileTreeNode({ entry, depth, selectedPath, onSelectFile, readDir }: {
  entry: FsEntry;
  depth: number;
  selectedPath?: string;
  onSelectFile: (entry: FsEntry) => void;
  ...
  return <div role="treeitem" aria-expanded={isDirectory ? expanded : undefined}>
    <button className={`file-tree-row${selected ? ' selected' : ''}`} style={{ paddingLeft: 8 + depth * 14 }} onClick={() => void activate()} title={entry.path}>
    ...
    {isDirectory && expanded && children?.map(child => <FileTreeNode key={child.path} entry={child} depth={depth + 1} selectedPath={selectedPath} onSelectFile={onSelectFile} readDir={readDir} />)}
  </div>;
}
