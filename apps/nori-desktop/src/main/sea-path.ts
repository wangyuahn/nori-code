...
export function currentTarget(): string {
  ...
  if (!SUPPORTED_TARGETS.has(target)) {
    throw new Error(`No bundled Nori server for this platform: ${target}`);
  }
  return target;
}

function executableName(): string {
  return process.platform === 'win32' ? 'nori.exe' : 'nori';
}

/**
 * Absolute path to the bundled SEA backend executable.
 *
 * - packaged: `<resources>/bin/<target>/nori[.exe]` — placed there by
 *   electron-builder `extraResources`.
 * - dev: `apps/nori-code/dist-native/bin/<target>/nori[.exe]` — produced by
 *   `pnpm -C apps/nori-code build:native:sea`. In dev `app.getAppPath()` is
 *   `apps/nori-desktop`, so the sibling app is one level up.
 */
