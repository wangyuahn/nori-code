...
/**
 * Fetch the latest published Nori Code version from the CDN.
 *
...
export async function fetchLatestVersionFromCdn(
  ...
  const response = await fetchWithTimeout(fetchImpl, NORI_CODE_CDN_LATEST_URL);
  if (!response.ok) {
...
async function fetchUpdateManifestFromCdn(fetchImpl: typeof fetch): Promise<UpdateManifest> {
  const response = await fetchWithTimeout(fetchImpl, NORI_CODE_CDN_LATEST_JSON_URL);
  if (!response.ok) {
