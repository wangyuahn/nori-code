...
describe('sanitizeShellOutput', () => {
  ...
  it('strips DCS and other string controls including their payloads', () => {
    expect(sanitizeShellOutput(`a${ESC}Pqpayload${ESC}\\b`)).toBe('ab');
    expect(sanitizeShellOutput(`a${ESC}_payload${ESC}\\b`)).toBe('ab');
  });

  it('strips 8-bit C1 control sequences', () => {
    expect(sanitizeShellOutput(`a\u009B2Jb`)).toBe('ab');
    expect(sanitizeShellOutput(`a\u009D0;title\u009Cb`)).toBe('ab');
  });

  it('strips unterminated terminal string sequences to the end', () => {
    expect(sanitizeShellOutput(`safe${ESC}]0;bad title`)).toBe('safe');
    expect(sanitizeShellOutput(`safe${ESC}Pbad payload`)).toBe('safe');
  });
