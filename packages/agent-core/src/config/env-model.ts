...
export const ENV_MODEL_PROVIDER_KEY = '__nori_env__';
export const ENV_MODEL_ALIAS_KEY = '__nori_env_model__';
...
/** Default context window (256K) used when NORI_MODEL_MAX_CONTEXT_SIZE is unset. */
const DEFAULT_MAX_CONTEXT_SIZE = 262144;

/** Default capabilities when NORI_MODEL_CAPABILITIES is unset. */
...
function parseProviderType(raw: string | undefined): ProviderType {
  ...
  if (!ALLOWED_TYPES.includes(normalized)) {
    fail(
      `NORI_MODEL_PROVIDER_TYPE must be one of ${ALLOWED_TYPES.join(', ')}, got "${raw}".`,
    );
  }
  return normalized;
}
...
// fails fast like the other NORI_MODEL_* values, instead of silently keeping
...
/**
 * When `NORI_MODEL_NAME` is set, synthesize one provider + one model alias from
 * the `NORI_MODEL_*` environment variables and make it the default model.
...
export function applyEnvModelConfig(config: KimiConfig, env: Env = process.env): KimiConfig {
  const model = trimmed(env['NORI_MODEL_NAME']);
  if (model === undefined) return config;

  const apiKey = trimmed(env['NORI_MODEL_API_KEY']);
  if (apiKey === undefined) {
    fail('NORI_MODEL_NAME is set but NORI_MODEL_API_KEY is missing.');
  }

  const maxContextRaw = trimmed(env['NORI_MODEL_MAX_CONTEXT_SIZE']);
  const maxContextSize =
    maxContextRaw === undefined
      ? DEFAULT_MAX_CONTEXT_SIZE
      : parsePositiveInt(maxContextRaw, 'NORI_MODEL_MAX_CONTEXT_SIZE');

  const type = parseProviderType(trimmed(env['NORI_MODEL_PROVIDER_TYPE']));
  const baseUrl = trimmed(env['NORI_MODEL_BASE_URL']) ?? DEFAULT_BASE_URL[type];
  ...
  const maxOutputRaw = trimmed(env['NORI_MODEL_MAX_OUTPUT_SIZE']);
  const maxOutputSize =
    maxOutputRaw !== undefined
      ? parsePositiveInt(maxOutputRaw, 'NORI_MODEL_MAX_OUTPUT_SIZE')
      : undefined;
  const capabilities = parseCapabilities(env['NORI_MODEL_CAPABILITIES']) ?? DEFAULT_CAPABILITIES;
  const displayName = trimmed(env['NORI_MODEL_DISPLAY_NAME']);
  const reasoningKey = trimmed(env['NORI_MODEL_REASONING_KEY']);
  const adaptiveThinking = parseBooleanVar(
    env['NORI_MODEL_ADAPTIVE_THINKING'],
    'NORI_MODEL_ADAPTIVE_THINKING',
  );
  ...
  const thinkingEffort = trimmed(env['NORI_MODEL_THINKING_EFFORT']);
