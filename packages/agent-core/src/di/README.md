# `@nori-code/agent-core` Dependency Injection Container
...
- **Testing** — `TestInstantiationService` (subpath
  `@nori-code/agent-core/di/test`) exposes direct `.get` / `.stub` so
...
Test files import from the subpath `@nori-code/agent-core/di/test`
(NOT the main package entry — keeps production bundles clean):

```ts
import { TestInstantiationService } from '@nori-code/agent-core/di/test';
...
├── test.ts                      ← subpath barrel (`@nori-code/agent-core/di/test`)
