...
export async function resolveMcpJsonPaths(input: ResolveMcpJsonPathsInput): Promise<McpJsonPaths> {
  ...
  return {
    ...
    project: join(input.cwd, '.nori-code', 'mcp.json'),
  };
}
...
/**
 * Load MCP server declarations from the user-global `~/.nori-code/mcp.json`,
 * the project-root `<project root>/.mcp.json`, and the project-local
 * `<cwd>/.nori-code/mcp.json`. Entries in later files override earlier files
