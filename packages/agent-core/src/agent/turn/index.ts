...
export const NORI_REVIEW_GATE_REMINDER_NAME = 'nori_review_gate';
...
export class TurnFlow {
  ...
  /** NORI: note types written during the current phase (for phase-switch gate). */
  ...
  private async runOneTurn(
    ...
    this.turnToolNames = [];
    this.turnFilesCreated = 0;
    this.turnFilesModified = 0;
          ...
          hooks: {
            beforeStep: async ({ signal: stepSignal }) => {
              ...
              if (ruleEngine) {
                ...
                const cleaned = removeNoriRulesBlock(currentPrompt);
                if (rulePrompt) {
                  this.agent.config.update({ systemPrompt: cleaned + '\n\n' + rulePrompt });
                } else {
                  // No rules active — clean up old injection without appending new block
            ...
            afterStep: async ({ usage, stopReason }) => {
              this.agent.usage.record(model, usage, 'turn');
              await this.agent.fullCompaction.afterStep();
              deduper.endStep();

              // A steered message (TeamDM, team Chat, background notification) is
              // flushed by `beforeStep` before every LLM call, so it reaches the
              // model at the next tool-call boundary without waiting for the turn
              // to end on its own.
              ...
              if (
                ...
                if (workflowGatePrompt !== undefined) {
                  this.agent.context.appendUserMessage(
                    [{ type: 'text', text: workflowGatePrompt }],
                    { kind: 'system_trigger', name: NORI_REVIEW_GATE_REMINDER_NAME },
                  );
            ...
            shouldContinueAfterStop: async (ctx) => {
              ...
              if (this.pendingNoriWorkflowGateContinuation) {
                this.pendingNoriWorkflowGateContinuation = false;
                if (!hasStepBudgetRemaining(loopControl?.maxStepsPerTurn, ctx.stepNumber)) {
                  this.agent.context.popMatchedMessage(isNoriReviewGateReminderOrigin);
                  return { continue: false };
                }
                return { continue: true };
              }
            ...
            prepareToolExecution: async (ctx) => {
              ...
              // Track nori_memory_write note_type for phase-switch gate
              if (ctx.toolCall.name === 'nori_memory_write') {
  ...
  private trackToolLifecycle(event: LoopEvent, turnId: number): void {
    if (event.type === 'tool.call') {
      ...
      if (event.name === 'Bash') {
        ...
        if (isVerificationCommand(event.args)) {
          this.turnVerificationCommandCount++;
        }
      }
      if (event.name === 'Write') {
        this.noriLastPhase = 'implement';
        this.turnFilesCreated++;
        const filePath = toolFilePath(event.args);
        if (isTestFilePath(filePath)) {
          this.turnTestFilesCreated++;
        }
      }
      if (event.name === 'Edit') {
        this.noriLastPhase = 'implement';
        this.turnFilesModified++;
        const filePath = toolFilePath(event.args);
        if (isTestFilePath(filePath)) {
          this.turnTestFilesCreated++;
        }
  ...
  private buildNoriWorkflowGatePrompt(): string | undefined {
    const workflow = this.agent.noriWorkflow;
    if (workflow === undefined) return undefined;

    const decision = decideNoriWorkflowGate(workflow, this.noriWorkflowActivity());
    if (decision === undefined) return undefined;

    const gateKey = decision.kind;
    const continuationCount = this.noriWorkflowGateContinuationCounts.get(gateKey) ?? 0;
    const maxContinuations =
      decision.mode === 'suggested' ? 1 : workflow.maxReviewGateContinuations;
    if (continuationCount >= maxContinuations) {
      return undefined;
    }

    this.noriWorkflowGateContinuationCounts.set(gateKey, continuationCount + 1);
    this.noriLastPhase = decision.phase;
    return this.renderNoriWorkflowGatePrompt(decision);
  }

  private noriWorkflowActivity(): NoriWorkflowActivity {
    return {
      ...this.noriReviewActivity(),
      memorySearchCount: this.turnMemorySearchCount,
      memoryWriteCount: this.turnMemoryWriteCount,
      userPromptText: this.turnUserPromptText,
    };
  }

  private noriReviewActivity(): NoriReviewActivity {
    return {
      filesCreated: this.turnFilesCreated,
      filesModified: this.turnFilesModified,
  ...
  private renderNoriWorkflowGatePrompt(decision: NoriWorkflowGateDecision): string {
    if (decision.kind === 'review' && decision.review !== undefined) {
      return this.renderNoriReviewGatePrompt(decision.review);
    }

    const activity = this.noriWorkflowActivity();
    ...
    lines.push('</nori_workflow_gate>');
    return lines.join('\n');
  }

  private renderNoriReviewGatePrompt(decision: NoriReviewGateDecision): string {
    const activity = this.noriReviewActivity();
    const lines = [
      `<nori_review_gate mode="${decision.mode}" difficulty_score="${decision.score}" suggested_threshold="${decision.suggestionThreshold}" required_threshold="${decision.requiredThreshold}">`,
      `Reason: ${decision.reason}.`,
      '',
    ...
    if (decision.mode === 'required') {
      ...
      lines.push(
        '',
        'Review is suggested, not mandatory. If your own forward difficulty assessment reaches the required threshold, run the review now. Otherwise either ask the user whether to run a review when AskUserQuestion is available, or finish with a concise note that review was optional and not forced.',
      );
    }

    lines.push('</nori_review_gate>');
    return lines.join('\n');
  }
  ...
  /**
   * Check nori note-writing rules against the note types written in the current
   * phase. Returns a human-readable string listing missing note types, or null if
  ...
  private checkNoriNoteRules(
    ...
    return (
      `Note-writing rules require you to document your work before leaving the "${phaseName}" phase. ` +
      `Missing note type(s): ${labels}. ` +
      `Call nori_memory_write to write the required note(s), then the phase will advance.`
    );
  }

  /**
   * Inject a note-requirement system-reminder into the agent's system prompt,
   * so the model sees it on the next turn and is prompted to write the missing
   * notes before phase can advance.
   */
  private injectNoriNoteReminder(hint: string, _phaseName: string): void {
    const currentPrompt = this.agent.config.systemPrompt;
    const marker = '<system-reminder>';
    const block = `${marker}\n## Note Requirement Not Met\n\n${hint}\n</system-reminder>`;

    // Remove any existing note-requirement reminder (idempotent)
    const startTag = '<system-reminder>\n## Note Requirement Not Met';
    const endTag = '</system-reminder>';
    let cleaned = currentPrompt;
    const idx = cleaned.indexOf(startTag);
    if (idx !== -1) {
      const endIdx = cleaned.indexOf(endTag, idx);
      cleaned = endIdx === -1
        ? cleaned.substring(0, idx).trimEnd()
        : (cleaned.substring(0, idx) + cleaned.substring(endIdx + endTag.length)).trimEnd();
    }
...
/** Remove any previously injected NORI rules block (idempotent). */
function removeNoriRulesBlock(prompt: string): string {
  // Match both mandatory and advisory rules blocks: they all start with
  // '<system>\nThe following **' followed by either 'mandatory rules' or 'advisory rules'.
  const startMarker = '<system>\nThe following **';
  const idx = prompt.indexOf(startMarker);
  if (idx === -1) return prompt;

  // Find the matching </system> — there may be two blocks (enforced + advisory).
  // Use lastIndexOf so both blocks are removed when present.
  const endMarker = '</system>';
  let endIdx = prompt.lastIndexOf(endMarker);
  if (endIdx === -1 || endIdx <= idx) return prompt;

  return (prompt.substring(0, idx) + prompt.substring(endIdx + endMarker.length)).trimEnd();
}
...
function isNoriReviewGateReminderOrigin(origin: PromptOrigin | undefined): boolean {
  return (
    origin?.kind === 'system_trigger' &&
    origin.name === NORI_REVIEW_GATE_REMINDER_NAME
  );
}
...
function isTestFilePath(filePath: string): boolean {
  return /(?:^|[/\\])(?:__tests__|test|tests|spec)(?:[/\\]|$)|[._-](?:test|spec)\.[cm]?[jt]sx?$/i
    .test(filePath);
}

function isVerificationCommand(args: unknown): boolean {
  if (!isPlainRecord(args)) return false;
  const command = args['command'];
  if (typeof command !== 'string') return false;
  return /\b(?:test|tests|lint|typecheck|tsc|vitest|jest|pytest|cargo\s+test|go\s+test|pnpm\s+build|npm\s+run\s+build|yarn\s+build)\b/i
    .test(command);
}
