...
/**
 * Legacy compatibility helper. Context injections are durable user/reminder
 * messages and must stay that way in provider history; they are not model
 * tools. Older callers may still import this helper, so preserve its shape
 * while returning the original message unchanged.
 *
