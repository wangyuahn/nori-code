...
export class Agent {
  ...
  useProfile(
    ...
    this.permission.setToolsReadonly(profile.toolsReadonly ?? false);
  }
  ...
  get rpcMethods(): PromisableMethods<AgentAPI> {
    return {
      ...
      setNoriRuntimeSettings: (payload) => {
        if (payload.coderWriteEnabled !== undefined) {
          this.coderWriteEnabled = payload.coderWriteEnabled;
        }
