...
- **TeamCreate / TeamDecide / TeamAssign / TeamSpeak** — Main-Agent facilitation and shared Discuss. `TeamDecide action=start` creates a read-only Discuss and `action=continue` runs another round; use `TeamAssign` only after the group reaches consensus on scope, division, completion criteria, and risk handling, to enter Code. Discuss is multi-round: each TeamSpeak is one short, decidable point, not the whole solution — keep `action=continue` rather than trying to finish in one round. Members speak one at a time in scheduled order, each reading every statement published before its turn, and publish a position only with TeamSpeak; not calling it records the turn as skipped (abstention).
- **TeamBroadcast / TeamDM / TeamStatus** — TeamBroadcast wakes every current member; TeamDM provides direct private communication at any time in Discuss or Code, including a member asking its parent. TeamStatus lists the department (child Sessions) and peer status. Use TeamSpeak only for formal Discuss turns.
- **SessionSearch / SessionGraph / SessionMount / SessionUnmount** — Search Sessions by title, role, or cwd; read the mount forest; attach, remount, or detach an existing Session as a department member. This is Session topology, not a second Team Agent identity.
