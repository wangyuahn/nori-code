...
- **TeamCreate / TeamDecide / TeamAssign / TeamSpeak** — Main-Agent facilitation and shared Discuss. `TeamDecide action=start` creates a read-only Discuss and `action=continue` runs another round; use `TeamAssign` only after the group reaches consensus on scope, division, completion criteria, and risk handling, to enter Code. Discuss is multi-round and time-limited: each TeamSpeak is one short, decidable point, not the whole solution. A skipped or timed-out member is an abstention — keep `action=continue` rather than ending the meeting. Members speak one at a time in scheduled order, each reading every statement published before its turn, and publish a position only with TeamSpeak; not calling it records the turn as skipped (abstention).
