 ...
 *   { id: 'my_feature', title: 'My feature', description: '...', env: 'NORI_CODE_EXPERIMENTAL_MY_FEATURE', default: false, surface: 'both' }
 *
 * Keep the `as const satisfies` — it derives the literal `FlagId` union that gives `enabled()`
 * autocomplete and typo-checking. `env` must start with 'NORI_CODE_EXPERIMENTAL_', be unique, and
 * not equal the master switch 'NORI_CODE_EXPERIMENTAL_FLAG'; `id` must not be 'flag'.
 */
