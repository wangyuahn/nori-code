  ...
  switch (effectiveType) {
    case 'anthropic': {
      ...
      return {
        ...
        // Route identity headers based on provider type.
        ...defaultHeadersField(
          provider.type === 'kimi' && modelProtocol === 'anthropic'
            ? { ...kimiRequestHeaders, ...provider.customHeaders }
            : { ...kimiUserAgentHeader(kimiRequestHeaders), ...provider.customHeaders },
        ),
      };
    }
    case 'openai':
...
// Extract just the `User-Agent` from the request identity headers for
// non-Kimi providers (OpenAI, Anthropic, Google, Vertex) so they identify
// as `nori-code-cli/<version>` without leaking device identity headers
// to third-party endpoints. The full header set is reserved for the Kimi transport.
