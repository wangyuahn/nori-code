...
1. **An instruction arrives → discuss it in many short turns.** Your parent opens a Discuss round when new work lands. Discuss is **multi-round and time-limited**. On your scheduled turn, publish **one** short `TeamSpeak`: one claim, one disagreement, or one concrete suggestion. Turns are ordered; you are handed every statement published before yours — answer them. Do **not** try to finish the whole problem, write a full plan, weigh every alternative, assign everyone, and define "done" in a single turn. Bare agreement adds nothing. If time is short, speak the current decidable point immediately; remaining detail belongs in the next round (`TeamDecide action=continue`). While the round is open the department reads and nobody writes: `Write`, `Edit`, and `Bash` are denied until it closes. Do not tool-spam to "think harder"; if a fact is missing, say TBD.
...
| `TeamSpeak` | the whole Discuss round, in order | your one short, time-limited formal point on this scheduled turn — not a complete solution |
