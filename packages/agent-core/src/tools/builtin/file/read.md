...
- Output starts with `[path#TAG]`, where TAG is a four-hex hash of the complete file after BOM removal and LF line-ending normalization.
- File lines then use `<line-number>\t<content>`.
- Pass the latest TAG to `Edit.expected_tag`. After Edit succeeds, use its returned TAG or Read again before another edit.
...
- Mixed or lone carriage-return line endings are shown as `\r`; preserve those explicit characters in affected `Edit.line_ops` replacement content.
