...
function formatLineOperations(args: EditInput): string {
  const lines = [`Expected tag: ${args.expected_tag.toUpperCase()}`];
  for (const operation of args.line_ops) {
    switch (operation.op) {
      case 'swap':
        lines.push(`swap ${String(operation.start)}-${String(operation.end)}`);
        appendApprovalContent(lines, operation.content);
        break;
      case 'del':
        lines.push(`delete ${String(operation.start)}-${String(operation.end)}`);
        break;
      case 'insert_pre':
        lines.push(`insert before ${String(operation.line)}`);
        appendApprovalContent(lines, operation.content);
        break;
      case 'insert_post':
        lines.push(`insert after ${String(operation.line)}`);
        appendApprovalContent(lines, operation.content);
        break;
    }
  }
...
function appendApprovalContent(lines: string[], content: string): void {
  const contentLines = splitOperationContent(content);
