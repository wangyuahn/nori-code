...
function llmHistoryText(ctx: ReturnType<typeof testAgent>): string {
  return ctx.llmCalls
    .flatMap((call) => call.history)
...
describe('Agent turn flow', () => {
  it('continues with a suggested Nori review gate for simple implementation work', async () => {
    const ctx = testAgent({
      kaos: createCommandKaos(''),
      noriWorkflow: {
    ...
    ctx.mockNextResponse(
      ...
      writeCallWithId('call_write', 'src/simple.ts'),
    );
    ...
    expect(inputs).toContain('<nori_review_gate mode="suggested"');
    expect(inputs).toContain('Review is suggested, not mandatory.');
    expect(inputs).not.toContain('<nori_review_gate mode="required"');
  });

  it('continues with a required Nori review gate when difficulty reaches the threshold', async () => {
    const ctx = testAgent({
      kaos: createCommandKaos(''),
      noriWorkflow: {
        reviewSuggestionThreshold: 4,
    ...
    ctx.mockNextResponse(
      { type: 'text', text: 'I will write several files.' },
      writeCallWithId('call_write_a', 'src/a.ts'),
      writeCallWithId('call_write_b', 'src/b.ts'),
      writeCallWithId('call_write_c', 'src/c.ts'),
    );
    ctx.mockNextResponse({ type: 'text', text: 'Implementation is done.' });
    ctx.mockNextResponse({ type: 'text', text: 'I cannot run review in this test harness.' });

    await ctx.rpc.prompt({ input: [{ type: 'text', text: 'Add several files' }] });
    await ctx.untilTurnEnd();

    expect(ctx.llmCalls).toHaveLength(3);
    const inputs = llmHistoryText(ctx);
    expect(inputs).toContain('<nori_review_gate mode="required"');
    expect(inputs).toContain('Review is mandatory now.');
  ...
  it('continues with a required Nori memory-search gate before implementation completion', async () => {
    ...
    ctx.mockNextResponse(
      { type: 'text', text: 'I will implement directly.' },
      writeCallWithId('call_write', 'src/memory-required.ts'),
    );
    ctx.mockNextResponse({ type: 'text', text: 'Implementation is complete.' });
    ctx.mockNextResponse({ type: 'text', text: 'Memory search is not available in this harness.' });

    await ctx.rpc.prompt({ input: [{ type: 'text', text: '实现这个改动' }] });
    await ctx.untilTurnEnd();

    expect(ctx.llmCalls).toHaveLength(3);
    const inputs = llmHistoryText(ctx);
    expect(inputs).toContain('<nori_workflow_gate kind="memory_search"');
    expect(inputs).toContain('Call nori_memory_search now with concrete keywords');
  });

  it('injects nori_memory_search results into the next model request', async () => {
      ...
      multiRetrieve: vi.fn(async () => [
        {
          ...
          score: 5,
          excerpt: 'Known memory context: coder_write_enabled is controlled by runtime settings.',
        },
      ]),
      writeNote: vi.fn(),
    ...
    ctx.configure({ tools: ['nori_memory_search'] });
    await ctx.rpc.setPermission({ mode: 'auto' });

    ctx.mockNextResponse(
      { type: 'text', text: 'I will search memory.' },
      noriMemorySearchCallWithId('call_memory', ['coder_write_enabled']),
    );
    ctx.mockNextResponse({ type: 'text', text: 'I found the prior decision.' });

    await ctx.rpc.prompt({ input: [{ type: 'text', text: 'Check coder write memory' }] });
    await ctx.untilTurnEnd();

    expect(ctx.llmCalls).toHaveLength(2);
    const secondHistory = ctx.llmCalls[1]?.history ?? [];
    const toolResult = secondHistory.find(
      (message) => message.role === 'tool' && message.toolCallId === 'call_memory',
    );
    ...
    expect(toolResult?.content).toEqual([
      expect.objectContaining({
        ...
        text: expect.stringContaining(
          'Known memory context: coder_write_enabled is controlled by runtime settings.',
        ),
      }),
    ]);
  });
...
function writeCallWithId(id: string, path: string): ToolCall {
  return {
    type: 'function',
    id,
    name: 'Write',
    arguments: JSON.stringify({ path, content: 'export const value = 1;\n' }),
  };
}

function noriMemorySearchCallWithId(id: string, keywords: string[]): ToolCall {
  return {
    type: 'function',
    id,
