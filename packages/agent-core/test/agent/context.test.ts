...
describe('Agent context', () => {
  ...
  it('projects context injection as a visible harness reminder, not a tool exchange', () => {
    const ctx = testAgent();
    ctx.configure();
  ...
  it('keeps system reminders separate from real user prompts', async () => {
    ...
    const injectionMessage = input.input.history.find(
      (message) => message.role === 'user' && message.content.some(
        (part) => part.type === 'text' && part.text.includes('Remember the host note.'),
      ),
    );
    expect(injectionMessage).toMatchObject({
      role: 'user',
  ...
  it('drops deferred reminders when compaction drops a pending tool exchange', async () => {
    ...
    expect(ctx.agent.context.messages.map((message) => message.role)).toEqual(['user', 'user', 'user']);
    await ctx.expectResumeMatches();
  });
...
describe('Agent context notification projection', () => {
  ...
  it('renders a closed attribute set and ignores unknown keys', () => {
    // `agent_id` used to be emitted so the LLM could feed an id back to the
    // temporary-subagent resume path. That path no longer exists, so the
    // attribute set is closed: an extra key must not leak into the tag.
    const text = renderNotificationXml({
      id: 'n_lost1',
      category: 'task',
      type: 'task.lost',
      source_kind: 'background_task',
      source_id: 'question-w7gq3wwj',
      agent_id: 'agent-0',
      title: 'Background question lost',
      severity: 'warning',
      body: 'Background question 1 lost.',
    });

    expect(text.split('\n')[0]).toBe(
      '<notification id="n_lost1" category="task" type="task.lost" source_kind="background_task" source_id="question-w7gq3wwj">',
    );
