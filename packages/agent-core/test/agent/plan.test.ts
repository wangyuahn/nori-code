...
function toolContext<Input>(args: Input, toolCallId = 'call_1') {
...
describe('plan exit tool', () => {
  it('reads the same session plan file written by nori_plan_write', async () => {
    const files = new Map<string, string>();
    const readText = vi.fn(async (path: string) => files.get(path) ?? '');
    const writeText = vi.fn(async (path: string, content: string) => {
      files.set(path, content);
    ...
    await ctx.agent.planMode.enter('nori-session-plan', false);
    ...
    const writeResult = await executeTool(
      new NoriPlanWriteTool(ctx.agent),
      toolContext({
        file_path: 'plans/model-requested-name.md',
        content,
      }),
    );
    ...
    await expect(ctx.rpc.getPlan({})).resolves.toMatchObject({
      id: 'nori-session-plan',
      content,
      path: planPath,
    });

    const exitResult = await executeTool(
      new ExitPlanModeTool(ctx.agent),
      toolContext({}),
    );

    expect(exitResult.isError).toBeFalsy();
    expect(readText).toHaveBeenCalledWith(planPath);
    expect(exitResult.output).toContain(`Plan saved to: ${planPath}`);
    expect(exitResult.output).toContain(content);
...
describe('plan allows safe tool flow', () => {
  ...
  it('allows read-only Bash to continue through permission and execution', async () => {
    ...
    expect(await ctx.untilTurnEnd()).toMatchInlineSnapshot(`
      ...
      [emit] agent.status.updated        { "model": "mock-model", "contextTokens": 0, "maxContextTokens": 1000000, "contextUsage": 0, "planMode": true, "swarmMode": false, "permission": "yolo", "coderWriteEnabled": false, "toolsReadonly": false, "maxSwarmDepth": 3 }
