...
describe('KimiCore plugin RPCs', () => {
  ...
  it('injects persisted managed Kimi Code environment into the datasource plugin MCP server', async () => {
    const previousBaseUrl = process.env['NORI_CODE_BASE_URL'];
    const previousCodeOAuthHost = process.env['NORI_CODE_OAUTH_HOST'];
    const previousOAuthHost = process.env['KIMI_OAUTH_HOST'];
    delete process.env['NORI_CODE_BASE_URL'];
    delete process.env['NORI_CODE_OAUTH_HOST'];
...
api_key = ""
oauth = { storage = "file", key = "oauth/nori-code-env-1234", oauth_host = "https://auth.dev.example.test" }
`,
        'utf8',
      );
      await writeFile(
      ...
      expect(mcpConfig.servers['plugin-kimi-datasource:data']?.env).toEqual(
        expect.objectContaining({
          NORI_CODE_BASE_URL: 'https://api.dev.example.test/coding/v1',
          NORI_CODE_OAUTH_HOST: 'https://auth.dev.example.test',
        }),
      );
    } finally {
      restoreEnv('NORI_CODE_BASE_URL', previousBaseUrl);
      restoreEnv('NORI_CODE_OAUTH_HOST', previousCodeOAuthHost);
