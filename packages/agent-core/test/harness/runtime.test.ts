...
`,
  ...
  // does not know the session yet, so reading `.nori-code/local.toml` through
  ...
  it('reads workspace local.toml through persistenceKaos during createSession', async () => {
    ...
    await mkdir(join(workDir, '.nori-code'), { recursive: true });
    await mkdir(sharedDir, { recursive: true });
    await writeFile(
      join(workDir, '.nori-code', 'local.toml'),
...
`,
  ...
  it('adds an additional dir through the session RPC', async () => {
    ...
    const localToml = await readFile(join(workDir, '.nori-code', 'local.toml'), 'utf-8');
    const session = core.sessions.get(created.id);
    const mainAgent = session?.getReadyAgent('main');
    ...
    expect(result).toMatchObject({
      ...
      projectRoot: workDir,
      configPath: join(workDir, '.nori-code', 'local.toml'),
      persisted: true,
    });
  ...
  it('adds a session-only additional dir without writing local.toml', async () => {
    ...
    await expect(readFile(join(workDir, '.nori-code', 'local.toml'), 'utf-8')).rejects.toThrow();
  });
