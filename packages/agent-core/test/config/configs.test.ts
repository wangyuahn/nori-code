...
`;
  ...
  it('round-trips loop limits and custom agent roles', async () => {
    ...
    await writeConfigFile(configPath, {
      ...
      customAgents: {
        reviewer: { description: 'Review risky changes', role: 'Find correctness bugs.', baseProfile: 'agent', model: 'deepseek-review', enabled: true },
      },
    });
...
describe('config path env override', () => {
  it('uses NORI_CODE_HOME when no explicit homeDir is supplied', () => {
    const saved = process.env['NORI_CODE_HOME'];
    try {
      process.env['NORI_CODE_HOME'] = '/tmp/kimi-from-env';
      ...
      if (saved === undefined) delete process.env['NORI_CODE_HOME'];
      else process.env['NORI_CODE_HOME'] = saved;
    }
  });
});
...
`);
  ...
  it('applies NORI_MODEL_* env overrides on top of a salvaged config', async () => {
    const configPath = await writeTempConfig(`${VALID_TOML}
[loop_control]
max_steps_per_turn = "nope"
`);
    const result = loadRuntimeConfigSafe(configPath, {
      NORI_MODEL_NAME: 'env-model',
      NORI_MODEL_API_KEY: 'sk-env',
      NORI_MODEL_MAX_CONTEXT_SIZE: '262144',
    });
    expect(result.envWarnings).toEqual([]);
    expect(result.config.models?.['__nori_env_model__']).toBeDefined();
    expect(result.config.providers['kimi']).toBeDefined();
    expect(result.fileWarnings).toHaveLength(1);
  });

  it('skips NORI_MODEL_* overrides with an env warning instead of throwing', async () => {
    const configPath = await writeTempConfig(VALID_TOML);
    const result = loadRuntimeConfigSafe(configPath, {
      NORI_MODEL_NAME: 'env-model',
    });
    ...
    expect(result.envWarnings[0]).toContain('NORI_MODEL');
