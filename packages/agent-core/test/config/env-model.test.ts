...
const MIN = {
  NORI_MODEL_NAME: 'kimi-for-coding',
  NORI_MODEL_API_KEY: 'sk-test',
  NORI_MODEL_MAX_CONTEXT_SIZE: '262144',
} as const;

describe('applyEnvModelConfig', () => {
  it('returns the config unchanged when NORI_MODEL_NAME is absent', () => {
    const base = getDefaultConfig();
    expect(applyEnvModelConfig(base, {})).toBe(base);
  });

  it('throws when NORI_MODEL_NAME is set but API key is missing', () => {
    expectConfigInvalid(() => apply({ NORI_MODEL_NAME: 'm' }));
  });

  it('defaults max_context_size to 262144 (256K) when unset', () => {
    expect(
      apply({ NORI_MODEL_NAME: 'm', NORI_MODEL_API_KEY: 'k' })
  ...
  it.each(['abc', '0', '1.5', '-1'])(
    ...
    (value) => {
      expectConfigInvalid(() =>
        apply({ ...MIN, NORI_MODEL_MAX_CONTEXT_SIZE: value }),
      );
    },
  );
  ...
  it('applies provider type and its default base url', () => {
    expect(apply({ ...MIN, NORI_MODEL_PROVIDER_TYPE: 'openai' })
      .providers[ENV_MODEL_PROVIDER_KEY]).toMatchObject({
      type: 'openai',
      baseUrl: 'https://api.openai.com/v1',
    });
    const anthropic = apply({ ...MIN, NORI_MODEL_PROVIDER_TYPE: 'anthropic' })
  ...
  it('rejects unsupported provider types', () => {
    expectConfigInvalid(() =>
      apply({ ...MIN, NORI_MODEL_PROVIDER_TYPE: 'google-genai' }),
    );
  });

  it('lets an explicit base url override the default', () => {
    expect(
      apply({ ...MIN, NORI_MODEL_BASE_URL: 'https://api.example.com/v1' })
  ...
  it('parses comma-separated capabilities (trimmed, lowercased)', () => {
    expect(
      apply({ ...MIN, NORI_MODEL_CAPABILITIES: 'Image_In, thinking ,' })
  ...
  it('sets display_name only when provided', () => {
    ...
    expect(
      apply({ ...MIN, NORI_MODEL_DISPLAY_NAME: 'Custom Model' })
  ...
  it('writes type-specific fields and validates max_output_size', () => {
    const alias = apply({
      ...MIN,
      NORI_MODEL_PROVIDER_TYPE: 'anthropic',
      NORI_MODEL_MAX_OUTPUT_SIZE: '8192',
      NORI_MODEL_REASONING_KEY: 'reasoning',
    }).models?.[ENV_MODEL_ALIAS_KEY];
    expect(alias?.maxOutputSize).toBe(8192);
    expect(alias?.reasoningKey).toBe('reasoning');
    expectConfigInvalid(() =>
      apply({ ...MIN, NORI_MODEL_MAX_OUTPUT_SIZE: 'nope' }),
    );
  });
  ...
  it('maps the thinking effort variable', () => {
    const config = apply({
      ...MIN,
      NORI_MODEL_THINKING_EFFORT: 'high',
    });
    expect(config.thinking).toMatchObject({ effort: 'high' });
    expect(config.thinking?.enabled).toBeUndefined();
  });

  it('maps NORI_MODEL_ADAPTIVE_THINKING onto the alias', () => {
    expect(
      apply({ ...MIN, NORI_MODEL_ADAPTIVE_THINKING: 'true' })
        .models?.[ENV_MODEL_ALIAS_KEY]?.adaptiveThinking,
    ).toBe(true);
    expect(
      apply({ ...MIN, NORI_MODEL_ADAPTIVE_THINKING: 'false' })
        .models?.[ENV_MODEL_ALIAS_KEY]?.adaptiveThinking,
    ).toBe(false);
    expect(
  ...
  it('rejects an invalid NORI_MODEL_ADAPTIVE_THINKING', () => {
    expectConfigInvalid(() =>
      apply({ ...MIN, NORI_MODEL_ADAPTIVE_THINKING: 'maybe' }),
    );
  });
...
describe('writeConfigFile never persists the env model', () => {
  ...
  it('output never contains env-injected identifiers', async () => {
    ...
    try {
      const runtime = loadRuntimeConfig(path, {
        ...MIN,
        NORI_MODEL_THINKING_EFFORT: 'low',
      });
      await writeConfigFile(path, runtime);
