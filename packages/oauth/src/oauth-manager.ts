...
export interface OAuthManagerOptions {
   ...
   * falls back to `process.env.NORI_CODE_HOME` so multi-process test
...
export class OAuthManager {
  ...
  constructor(options: OAuthManagerOptions) {
    ...
    // The `NORI_CODE_HOME` fallback MUST stay test-only so production
    ...
    const envConfigDir =
      process.env['NODE_ENV'] === 'test' ? process.env['NORI_CODE_HOME'] : undefined;
