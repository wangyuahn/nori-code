...
import { authSummarySchema, type AuthSummary } from '@nori-code/protocol';
...
beforeEach(() => {
  tmpDir = mkdtempSync(join(tmpdir(), 'nori-server-auth-test-'));
  lockPath = join(tmpDir, 'lock');
  bridgeHome = mkdtempSync(join(tmpdir(), 'nori-server-auth-home-'));
});
