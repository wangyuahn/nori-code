...
export interface HostCheckOptions {
  ...
  /** Extra allowed hosts / domain-suffix patterns (from `NORI_CODE_ALLOWED_HOSTS`). */
  readonly extra?: readonly string[];
  /** Disable the check entirely (`NORI_CODE_DISABLE_HOST_CHECK=1`; test-only). */
...
/**
 * Parse `NORI_CODE_ALLOWED_HOSTS` into an `extra` allowlist.
 *
...
export function parseAllowedHosts(env: NodeJS.ProcessEnv = process.env): string[] {
  const raw = env['NORI_CODE_ALLOWED_HOSTS'];
  if (raw === undefined) {
    return [];
  }
  return raw
...
/** True when `NORI_CODE_DISABLE_HOST_CHECK=1` (test/controlled env only). */
export function isHostCheckDisabled(env: NodeJS.ProcessEnv = process.env): boolean {
  return env['NORI_CODE_DISABLE_HOST_CHECK'] === '1';
}
...
export function formatHostErrorMessage(host: string | undefined): string {
  ...
  return `Invalid Host header: ${hostLabel}; allow this host with NORI_CODE_ALLOWED_HOSTS=${hostArg} or 'nori server run --allowed-host ${hostArg}'.`;
}
