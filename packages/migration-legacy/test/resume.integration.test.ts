 ...
 *     all from `@nori-code/agent-core/session/store`.
 *   - `Session` (constructor + `resume()` + `getReadyAgent()`), from
 *     `@nori-code/agent-core`; `localKaos` from `@nori-code/kaos`. After
...
import {
  ...
  encodeWorkDirKey,
  normalizeWorkDir,
} from '@nori-code/agent-core/session/store/index';
import { Session, type SDKSessionRPC } from '@nori-code/agent-core';
