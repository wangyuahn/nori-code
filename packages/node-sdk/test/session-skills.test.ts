...
describe('Session skills', () => {
  it('lists session skills without exposing content', async () => {
    ...
    try {
      ...
      expect(listed?.path.endsWith('/.nori-code/skills/review/SKILL.md')).toBe(true);
  ...
  it('activates a skill through core and emits the public skill event', async () => {
    ...
    try {
      ...
      const skillDir = normalizeWorkDir(await realpath(join(workDir, '.nori-code', 'skills', 'review')));
      await expect(
        waitForAgentWireEvent(
          homeDir,
          session.id,
        ...
        input: [
          {
            ...
            text: [
              ...
              '',
              `<nori-skill-loaded name="review" trigger="user-slash" source="project" dir="${skillDir}" args="src/app.ts">`,
              'Review the requested file.',
              '',
  ...
  it('resolves user brand skills from NORI_CODE_HOME, not the OS home', async () => {
    const homeDir = await makeTempDir(tempDirs, 'kimi-sdk-skills-home-');
...
async function writeLegacyUserSkill(
  ...
  await writeSkillFile(join(userHomeDir, '.nori-code', 'skills', name), name, description);
}
