...
export abstract class SDKRpcClientBase {
  ...
  async setNoriRuntimeSettings(
    input: SetSessionNoriRuntimeRpcInput,
  ): Promise<NoriRuntimeSettings> {
    const rpc = await this.getRpc();
    return rpc.setNoriRuntimeSettings({
      sessionId: input.sessionId,
      agentId: this.interactiveAgentId,
  ...
  async getNoriRuntimeSettings(input: SessionIdRpcInput): Promise<NoriRuntimeSettings> {
    const rpc = await this.getRpc();
    return rpc.getNoriRuntimeSettings({
      sessionId: input.sessionId,
      agentId: this.interactiveAgentId,
    });
  }
  ...
  async getStatus(input: SessionIdRpcInput): Promise<SessionStatus> {
    ...
    const noriRuntime = await rpc.getNoriRuntimeSettings({
      sessionId: input.sessionId,
      agentId,
    });
